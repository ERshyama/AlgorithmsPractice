...
def get_height(node):
    ...
    for child in node.children:
        depth, explored2 = get_height(child)
        for exp in explored2:
            explored.add(exp)
        if depth > max_depth:
            max_depth = depth
    return max_depth + 1, explored
...
def maxEnvelopes(envelopes):
    ...
    for node1 in all_nodes:
        for node2 in all_nodes:
            if node1.val[0] > node2.val[0] and node1.val[1] > node2.val[1]:
                if node1.children is None:
    ...
    for node in all_nodes:
        if node in explored:
            continue
        depth, explored2 = get_height(node)
        for exp in explored2:
            explored.add(exp)
        if depth > max_envs:
            max_envs = depth
